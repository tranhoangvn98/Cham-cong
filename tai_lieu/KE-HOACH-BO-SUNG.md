...
| 5 | Gửi yêu cầu OT, duyệt xong mới tính | **XONG hoàn toàn (17/09/2026)** — `quy_tac_tinh_cong.ts` tính `phut_ot` = giao của giờ có mặt và đơn `lam_them` **đủ hai cấp duyệt + kết quả OT (ảnh) được TBKS/Admin duyệt**; không có đơn thì OT = 0 kèm chú thích. Duyệt kết quả tự tính lại bảng công; hệ số Điều 98 (150/200/300%) xong ở v1.92.0. Luồng: đăng ký (thời gian + lý do + tài liệu tùy chọn) → trưởng bộ phận → TBKS/Admin → nộp ảnh kết quả → TBKS/Admin duyệt → vào lương | Không còn việc tồn đọng |
