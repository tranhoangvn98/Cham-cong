...
> Trạng thái: **THIẾT KẾ (chờ duyệt để triển khai)** — 2026-09-22
>
> Mục tiêu: khi một nhân viên nghỉ việc (thủ công hoặc tự động theo quyết định), hệ thống chấm
> công gửi webhook sang **ERP1** để ERP1 tự **vô hiệu hóa tài khoản** của người đó — không cần
> nhân sự thao tác tay bên ERP1.
...
| --- | --- | --- |
| `ms365.*` | Microsoft Graph | chặn đăng nhập + rút giấy phép khi nghỉ việc |
...
### 2.1 Loại sự kiện mới

```
erp1.nhan_su.nghi_viec
```
...
- Cổng phân quyền vẫn nhận `nhan_su.nghi_viec` như cũ.
- Microsoft Graph vẫn nhận `ms365.nghi_viec` như cũ.
...
## 3. Danh sách tệp dự kiến sửa

| Tệp | Việc |
| --- | --- |
| `may_chu/src/cau_hinh.ts` | thêm nhánh `erp1` |
| `may_chu/src/su_kien/hop_thu_di.ts` | loại `erp1.nhan_su.nghi_viec`, `di_sang_erp1()`, `gui_sang_erp1()`, `co_dich()` |
| `may_chu/src/nhan_su/nghi_viec.ts` | `returning ma_erp, email` + ghi thêm sự kiện |
| `.env.example` | thêm `ERP1_WEBHOOK_URL`, `ERP1_WEBHOOK_SECRET` |
| `may_chu/test/erp1_webhook.test.ts` | test đơn vị: dựng thân + định tuyến + cấu hình tắt (không gọi HTTP thật) |
| `may_chu/package.json` | đăng ký tệp test mới vào script `test` (gate `moi_test_deu_chay` bắt buộc) |
| `tai_lieu/WEBHOOK-ERP1.md` | tài liệu này |
| `CHANGELOG.md` | mục mới + tăng phiên bản |

## 4. Kiểm thử

1. **Đơn vị**: payload đúng hình dạng; `di_sang_erp1('erp1.nhan_su.nghi_viec') === true` và
...
2. **E2E cục bộ/VPS**: dựng một máy nhận webhook tạm (hoặc `webhook.site`), cho một nhân viên thử
   nghỉ việc → kiểm tra nhận đúng JSON + chữ ký khớp; gửi lại thủ công → `su_kien_id` không đổi.
3. **Bên ERP1**: xác minh chữ ký, `unique(su_kien_id)`, map `ma_erp`/`email` → tài khoản, set
   inactive. Cần phía ERP1 cung cấp URL nhận + cách họ khớp tài khoản (mã gì).
