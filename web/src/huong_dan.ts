...
export const HUONG_DAN: readonly HuongDanTrang[] = [
  ...
  },
  {
    ...
    tom_tat: 'Mỗi đợt phát hành, AI gom các mục mới trong CHANGELOG soạn thành VĂN BẢN CÔNG TY đúng thể thức NĐ30; admin công bố → cấp số, email toàn công ty + popup khi đăng nhập.',
    buoc: [
      { chu: 'Bấm Tạo dự thảo phát hành; hệ thống đề xuất sẵn khoảng phiên bản chưa từng công bố.' },
      { chu: 'AI soạn văn xuôi, hệ thống dựng thành văn bản NĐ30 (có DOCX, qua cổng kiểm tra) — y hệt soạn văn bản công ty.' },
      { chu: 'Bấm Soạn văn bản để xem trước, sửa văn xuôi hoặc viết lại bằng AI trong trình soạn.', vai_tro: NHAN_SU },
      { chu: 'Trình ký nếu cần, rồi admin bấm Công bố: cấp số, lưu vào Văn bản công ty, gửi email + popup toàn công ty.', vai_tro: NHAN_SU },
    ],
    luu_y: [
      'Văn bản phải ở trạng thái Chờ ký (toàn công ty cần trình ký trước) thì nút Công bố mới chạy — giống ban hành văn bản thường.',
