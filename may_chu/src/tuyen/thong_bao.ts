...
export async function tuyen_thong_bao(app: FastifyInstance): Promise<void> {
  ...
  app.post('/thong-bao', { preHandler: can_nhan_su }, async (req, res) => {
    ...
    // Gui email toi dung tap nguoi nhan (fire-and-forget; lich_chay quet lai neu mat).
    void gui_email_thong_bao(dong?.id ?? '').catch((loi) => {
      console.error('[thong_bao] gui email dang thong bao loi:', (loi as Error).message);
    });
