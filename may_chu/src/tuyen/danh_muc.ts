...
export async function tuyen_danh_muc(app: FastifyInstance): Promise<void> {
  ...
  app.get('/nhan-vien', { preHandler: can_dang_nhap }, async (req) => {
    ...
    return truy_van(
      `select nv.id, nv.ma_nv, nv.ho_ten, nv.pin_may, nv.ma_erp, nv.ngay_vao,
              nv.so_dien_thoai, nv.email, nv.chuc_danh, nv.vi_tri, nv.duoc_cham_cong_dien_thoai, nv.dang_hoat_dong,
  ...
  app.post('/nhan-vien', { preHandler: can_nhan_su }, async (req, res) => {
    ...
    const kq = await tao_ho_so_nhan_su({
      ma_nv: String(ts[0]),
      ho_ten: String(ts[1]),
      pin_may: ts[2] === null ? null : String(ts[2]),
      ma_erp: ts[3] === null ? null : String(ts[3]),
      phong_ban_id: ts[4] === null ? null : String(ts[4]),
      ca_lam_id: ts[5] === null ? null : String(ts[5]),
      ngay_vao: ts[6] === null ? null : String(ts[6]),
      so_dien_thoai: ts[7] === null ? null : String(ts[7]),
      email: ts[8] === null ? null : String(ts[8]),
      duoc_cham_cong_dien_thoai: ts[9] === true,
      noi_lam_viec_id: ts[10] === null ? null : String(ts[10]),
      che_do_luong: String(ts[11] ?? 'vn'),
      ...
      tao_tk_he_thong: luan_ly(b, 'tao_tk_he_thong', false) === true,
    });

    await ghi_nhat_ky(nguoi_dung_hien_tai(req).sub, 'tao_nhan_vien', 'nhan_vien',
      kq.id, { ma_nv: b['ma_nv'] }, req.ip);

    const ket_qua: Record<string, unknown> = { id: kq.id };
    ...
    if (kq.tai_khoan_he_thong !== null) {
      ket_qua['tai_khoan_he_thong'] = {
        ...
        ghi_chu: 'Tài khoản đăng nhập hệ thống được tạo tự động. Mật khẩu chỉ hiện lần này — '
          + 'nhân viên sẽ đổi ở lần đăng nhập đầu.',
      };
    }
