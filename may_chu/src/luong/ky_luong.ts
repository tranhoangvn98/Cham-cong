...
import { tach_phut_ot, type NgayOt } from './tach_phut_ot.ts';
...
/**
 * Tu thang nay tro di, viec mien 3 lan di muon/thang BAT BUOC phai co don xin phep duyet
 * truoc han (07:30). Truoc do (giai doan chuyen tiep) van mien tu dong khong can don.
 *
 * So sanh chuoi 'YYYY-MM' — dung truc tiep vi dinh dang co dinh, khop thu tu thoi gian.
 */
const THANG_BAT_BUOC_DON_DI_MUON = '2026-11';
  ...
  return {
    ...
    cs: {
      ...
      di_muon: {
        ...
        // Giai doan chuyen tiep: truoc THANG_BAT_BUOC_DON_DI_MUON van mien tu dong khong can don.
        yeu_cau_don_mien: thang >= THANG_BAT_BUOC_DON_DI_MUON,
      },
    ...
    ts: {
      ...
      bac_thue: bac,
      he_so_ot_ngay_thuong: Number(d['he_so_ot_ngay_thuong'] ?? 1.5),
      he_so_ot_nghi_tuan: Number(d['he_so_ot_nghi_tuan'] ?? 2),
      he_so_ot_ngay_le: Number(d['he_so_ot_ngay_le'] ?? 3),
    },
  };
}
...
interface DongNhanVien {
  ...
  lich_nghi_ma: string;
  /** Moc phat 50k RIENG cua nguoi nay ('HH:MM:SS'). Null = dung moc chung cong ty. */
...
export async function tinh_ky_luong(ky_luong_id: string, thang: string): Promise<number> {
       ...
       left join lateral (
         ...
         select coalesce(sum(least(so_cong,
                   case when exists(select 1 from buoi_lam_bu bl where bl.ngay = bang_cong_ngay.ngay)
                          then 1
                        when extract(dow from ngay) = 6
                          -- T7: khoi ghi de (null = theo tham so chung $3).
  ...
  // Phut OT THEO TUNG NGAY trong ky — de tach thanh ba loai (ngay thuong / CN / le) theo
  ...
  for (const r of await truy_van<{ nhan_vien_id: string; ngay: string; phut_ot: number }>(
    ...
    if (ds_ot === undefined) ot_theo_nguoi.set(r.nhan_vien_id, [{ ngay: r.ngay, phut_ot: r.phut_ot }]);
    else ds_ot.push({ ngay: r.ngay, phut_ot: r.phut_ot });
  }
  ...
  await trong_giao_dich(async (khach) => {
    ...
    for (const nv of ds) {
      ...
      // Moc phat RIENG cua nguoi nay (khai o ho so nhan su) ghi de moc chung cong ty; khong
      // khai thi dung moc chung. Vd ERP100: ca vao 08:30 -> phat 50k tu 08:40, nua ngay tu 09:00.
      const cau_hinh_dm: CauHinhDiMuon = {
        ...ts.cs.di_muon,
        moc_50k_phut: nv.di_muon_moc_50k !== null
          ? gio_sang_phut(nv.di_muon_moc_50k) : ts.cs.di_muon.moc_50k_phut,
        moc_nua_ngay_phut: nv.di_muon_moc_nua_ngay !== null
          ? gio_sang_phut(nv.di_muon_moc_nua_ngay) : ts.cs.di_muon.moc_nua_ngay_phut,
      };
      const muon = mien_phat
        ? { so_lan_50k_phat: 0, tien_50k: 0, so_lan_nua_ngay: 0 }
        : tinh_phat_di_muon(muon_theo_nguoi.get(nv.nhan_vien_id) ?? [], cau_hinh_dm);
      ...
      // OT tach theo LOAI NGAY (thuong / CN / le theo lich cua nguoi nay) + he so ngay thuong
      // theo khoi (kho_hn = 1.00 — ngoai le da duyet 17/09/2026; con lai theo tham so chung).
      const ot = tach_phut_ot(ot_theo_nguoi.get(nv.nhan_vien_id) ?? [], le_cua(nv.lich_nghi_ma));
      const he_so_ot_thuong = nv.he_so_ot_thuong_khoi === null
        ? (ts.ts.he_so_ot_ngay_thuong ?? 1.5)
        : Number(nv.he_so_ot_thuong_khoi);
      ...
      const kq = tinh_phieu_luong({
        ...
        phut_ot: nv.phut_ot,
        he_so_ot: he_so_ot_thuong,
        phut_ot_nghi_tuan: ot.nghi_tuan,
        phut_ot_le: ot.le,
        thuong,
        phu_cap_khac,
      ...
      await khach.query(
        `update phieu_luong set
           ...
           luong_net = $34,
           phut_ot_nghi_tuan = $35, phut_ot_le = $36,
           tien_ot_thuong = $37, tien_ot_nghi_tuan = $38, tien_ot_le = $39,
           he_so_ot_nghi_tuan = $40, he_so_ot_le = $41,
           tinh_luc = now()
         where id = $1`,
        [
          ...
          luong_net,
          ot.nghi_tuan, ot.le,
          kq.tien_ot_thuong, kq.tien_ot_nghi_tuan, kq.tien_ot_le,
          ts.ts.he_so_ot_nghi_tuan ?? 2, ts.ts.he_so_ot_ngay_le ?? 3,
        ],
      );
