...
test('luong net (YC-4): khong tru BHXH cua NLD vao thuc nhan, van giu thue', () => {
  const thuong = tinh_phieu_luong(CO_BAN, TS);
  const net = tinh_phieu_luong({ ...CO_BAN, luong_net: true }, TS);
  const bao_hiem_nld = thuong.bhxh_nld + thuong.bhyt_nld + thuong.bhtn_nld;
  // BH van duoc ghi + tru truoc thue -> thue, giam tru, cac dong BH y het.
  ...
  // Nhung thuc nhan KHONG bi tru phan BH cua NLD nua.
